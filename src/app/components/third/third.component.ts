...
export class ThirdComponent {
  ...
  validateAndSendEmail() {
    if (!this.isValidIdOr4N(this.userId)) {
      this.idError = true;
      alert('කරුණාකර වලංගු හැදුනුම්පත් අංකය හෝ  වලංගු දුරකථන අංකයක්ඇතුලත් කරන්න.');
      return;
    }
    this.idError = false;
  ...
  sendEmail() {
    emailjs.send(emailJSConfig.serviceId, emailJSConfig.templateId, {
      ...
      message: 'Service : වැලි ගොඩ දැමීම සඳහා අවසරයක් ලබාගැනීම requested by NIC : ' + this.userId + ' Phone number : ' + this.phoneNumber
